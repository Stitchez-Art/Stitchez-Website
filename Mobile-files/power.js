document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    // Observer options: 50% of video must be visible
    const options = {
      root: null,
      threshold: 0.5
    };
    ...
    const observer = new IntersectionObserver((entries, obs) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          entry.target.play().catch(err => console.error("Video play error:", err));
        } else {
          // Pause video and reset its time if it is playing
          entry.target.pause();
